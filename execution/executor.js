...
export const executeTrade = async ({ executor, amountIn, expectedProfit, minOut }) => {
  try {
    if (expectedProfit < 2 || amountIn > 0.05 || !minOut) return;
    ...
    const tx = await executor.router.populateTransaction.exactInputSingle({
      ...
      amountOutMinimum: ethers.utils.parseUnits(minOut.toFixed(2), 6),
      sqrtPriceLimitX96: 0
    });
    ...
    console.log("📦 Trade sent (protected)");
